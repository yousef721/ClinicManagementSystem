...
namespace CMS.Perestation.Layer.Areas.Admin.Controllers.CuraHub.Pharmacy
{
    ...
    public class PharmacyCategoryController : Controller
    {
        ...
        public IActionResult Edit(PharmacyCategoryVM pharmacyCategoryVM, int pageNumber = 0)
        {
            ...
            var oldCategory = _unitOfWork.PharmacyCategoryRepository.RetriveItem(e => e.Id == pharmacyCategoryVM.Id, trancked: false);
            if (oldCategory == null) return NotFound();

            if (ModelState.IsValid)
            {
                pharmacyCategoryVM.Img = HandleImageUpload(pharmacyCategoryVM.File, oldCategory?.Img);
