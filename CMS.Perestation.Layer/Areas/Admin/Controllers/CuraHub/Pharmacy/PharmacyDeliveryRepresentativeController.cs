...
namespace CMS.Perestation.Layer.Areas.Admin.Controllers.CuraHub.Pharmacy
{
    ...
    public class PharmacyDeliveryRepresentativeController : Controller
    {
        ...
        private string HandleImageUpload(IFormFile file, string oldImagePath = null)
        {
            ...
            var filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Files", "Images", "PharmacyDelivery", fileName);
            ...
            {
                var oldFilePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Files", "Images", "PharmacyDelivery", oldImagePath);
        ...
        public async Task<IActionResult> Create(PharmacyDeliveryRepresentativeVM PharmacyDeliveryVM)
        {
            ...
            {
                PharmacyDeliveryVM.ProfilePicture = HandleImageUpload(PharmacyDeliveryVM.FileProfile);
                PharmacyDeliveryVM.PersonalNationalIDCard = HandleImageUpload(PharmacyDeliveryVM.FileNationalIDCard);
        ...
        public async Task<IActionResult> Edit(PharmacyDeliveryRepresentativeVM PharmacyDeliveryVM, int pageNumber = 0)
        {
            ...
            if (ModelState.IsValid)
            {
                PharmacyDeliveryVM.ProfilePicture = HandleImageUpload(PharmacyDeliveryVM.FileProfile, oldDelivery.ProfilePicture);
                PharmacyDeliveryVM.PersonalNationalIDCard = HandleImageUpload(PharmacyDeliveryVM.FileNationalIDCard, oldDelivery.PersonalNationalIDCard);
        ...
        public async Task<IActionResult> Delete(int id, int pageNumber = 0)
        {
            ...
            if (delivery == null) return NotFound();
            // Delete Photo
            var imagePathProfile = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Files", "Images", "PharmacyDelivery", delivery.ProfilePicture);
            var imagePathId = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Files", "Images", "PharmacyDelivery", delivery.PersonalNationalIDCard);
